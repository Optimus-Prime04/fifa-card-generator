# FIFA Card Generator ⚽

This is a FIFA card generator I made using HTML, CSS, and JavaScript.

User can:
- upload a player image
- enter player stats
- generate your own FIFA card
- download the card as a PNG 

## Live Website

https://fifa-card-generator-gamma.vercel.app/

## Card Types

Currently i have added:
- Gold Card
- Icon Card
- UCL Card
## 📸 Preview
<img width="624" height="861" alt="Image" src="https://github.com/user-attachments/assets/f9366cea-eccd-4631-a8ee-c491cab02e10" />

<img width="624" height="846" alt="Image" src="https://github.com/user-attachments/assets/5c8077d9-b0c1-4f2d-ac67-67c75cb72536" />

<img width="624" height="832" alt="Image" src="https://github.com/user-attachments/assets/e964a842-36a3-41aa-8fff-638e2c523eb4" />


I Soon work to get more cards :)

## Built With

- HTML
- CSS
- JavaScript
- html2canvas
- GitHub
- Vercel

## About This Project and AI declaration 

I made this project as a student who loves playing fifa,i always wonder how great it would be if i had my own fifa card so i decided to create this.
i have also submitted this project on horizon, but when I submitted it  was only one type of card. Now I have updated it to 3 and also improved the UI .

One difficult part for me was making the PNG download feature,I Used AI suggestions for that part and then added it into my own project.

And some of the Most irritating ): parts were styling, positioning, debugging, and card adjustments were done through trial and error.


## Developer

Made by Aadhithya 🚀

## Project Structure

fifa-card-generator/

- index.html
- style.css
- script.js

assets/
- gold.png
- icon.png
- ucl.png

## License

MIT License
